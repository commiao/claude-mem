import { serializeObservationField, normalizeToolResult, ObservationPreparationError } from './observation-field.js';
...
// Operational size limit for serialized <parameters>/<outcome> content, not
// a model token/context capacity. Strict Claude admission rejects overflow;
// legacy callers retain the previous explicitly marked head/tail behavior.
export const OBS_PROMPT_FIELD_MAX_CHARS = 16_000;
const OBS_PROMPT_FIELD_HEAD_RATIO = 0.6;
const OBS_PROMPT_FIELD_TAIL_RATIO = 0.3;

function truncateObservationField(value: unknown, maxChars: number = OBS_PROMPT_FIELD_MAX_CHARS, strict = false): string {
  ...
  if (strict) throw new ObservationPreparationError('final-field-over-budget');
...
export function buildObservationPrompt(obs: Observation, strict = false): string {
  let toolInput: any;
  let toolOutput: any;
  ...
  <parameters>${truncateObservationField(toolInput, OBS_PROMPT_FIELD_MAX_CHARS, strict)}</parameters>
  <outcome>${truncateObservationField(normalizeToolResult(toolOutput), OBS_PROMPT_FIELD_MAX_CHARS, strict)}</outcome>
</observed_from_primary_session>
